...
func (r *Runtime) buildCommand() *urcli.Command {
	...
	return &urcli.Command{
		...
		Usage: "bootstrap cli",
		Flags: []urcli.Flag{
			&urcli.StringFlag{Name: "LOG_LEVEL", Value: r.logLevel, Destination: &r.logLevel, Sources: urcli.EnvVars("LOG_LEVEL")},
			&urcli.StringFlag{Name: "LOG_FORMAT", Value: r.logFormat, Destination: &r.logFormat, Sources: urcli.EnvVars("LOG_FORMAT")},
			&urcli.StringFlag{Name: "DATABASE_DSN", Value: r.databaseDSN, Destination: &r.databaseDSN, Sources: urcli.EnvVars("DATABASE_DSN")},
			&urcli.StringFlag{Name: "REDIS_ADDR", Value: r.redisAddress, Destination: &r.redisAddress, Sources: urcli.EnvVars("REDIS_ADDR")},
			&urcli.StringFlag{Name: "ASYNQ_QUEUE", Value: r.queueName, Destination: &r.queueName, Sources: urcli.EnvVars("ASYNQ_QUEUE")},
			&urcli.StringFlag{Name: "ASYNQ_RESULT_QUEUE", Value: r.resultQueueName, Destination: &r.resultQueueName, Sources: urcli.EnvVars("ASYNQ_RESULT_QUEUE")},
			&urcli.IntFlag{Name: "ASYNQ_CONCURRENCY", Value: r.asynqConcurrency, Destination: &r.asynqConcurrency, Sources: urcli.EnvVars("ASYNQ_CONCURRENCY")},
			&urcli.DurationFlag{Name: "ASYNQ_WAIT_TIMEOUT", Value: r.waitTimeout, Destination: &r.waitTimeout, Sources: urcli.EnvVars("ASYNQ_WAIT_TIMEOUT")},
			&urcli.DurationFlag{Name: "COPILOT_START_TIMEOUT", Value: r.copilotStartTimeout, Destination: &r.copilotStartTimeout, Sources: urcli.EnvVars("COPILOT_START_TIMEOUT")},
