...
func TestWorkerSessionStreamSubscriptionStartupDoesNotBlockOnReplayBacklog(t *testing.T) {
	const attempts = 5
	const replayEvents = 200
	blockedAttempts := 0

	for attempt := 1; attempt <= attempts; attempt++ {
		resolver := newControlPlaneResolverFixture(t)
		for index := 0; index < replayEvents; index++ {
			_, appendErr := resolver.StreamService.AppendAndPublish(context.Background(), domainstream.Event{
				EventID:    time.Now().UTC().Format("20060102150405.000000000") + "-worker-summary-" + string(rune('a'+(index%26))) + "-" + string(rune('0'+(index%10))),
				OccurredAt: time.Now().UTC(),
			...
			if appendErr != nil {
				t.Fatalf("append replay event attempt %d index %d: %v", attempt, index, appendErr)
			}
		}

		errCh := make(chan error, 1)
		go func() {
			_, subscriptionErr := (&subscriptionResolver{resolver}).WorkerSessionStream(
				context.Background(),
				models.SupervisorCorrelationInput{RunID: "", TaskID: "", JobID: ""},
				nil,
			)
			errCh <- subscriptionErr
		}()

		select {
		case subscriptionErr := <-errCh:
			if subscriptionErr != nil {
				t.Fatalf("attempt %d: unexpected subscription error: %v", attempt, subscriptionErr)
			}
	...
	if blockedAttempts > 0 {
		t.Fatalf("worker session subscription startup blocked on %d/%d attempts; replay backlog must not block subscription setup", blockedAttempts, attempts)
	}
}
